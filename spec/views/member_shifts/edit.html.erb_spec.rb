...
RSpec.describe('events/edit', type: :view) do
  ...
  it 'renders the edit event form' do
    ...
    assert_select 'form[action=?][method=?]', event_path(event), 'post' do
      ...
      assert_select 'input[name=?]', 'event[ideal_m1]'
      assert_select 'input[name=?]', 'event[ideal_m2]'
      assert_select 'input[name=?]', 'event[ideal_m3]'
      assert_select 'input[name=?]', 'event[ideal_m4]'
    end
  end
end
